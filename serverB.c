...
int main() {
   printf("The server B is up and running using UDP on port number %d\n", PORT);
   ...
   while(true) {
      ...
      // check if queried map ID is present in map
...
void formDataBuffer(bool found) {
   // if map ID is found, store its data in buffer
  if(found) {
...
bool readFile() {
   ...
   // open and read the map file content
   FILE *fptr;
